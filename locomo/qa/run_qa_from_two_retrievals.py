...
def run(args: argparse.Namespace) -> Path:
    ...
    for conv, sample_id in samples:
        ...
        try:
            ...
            merged, merge_meta = _merge_two_routes(
                conv=conv,
                sample_id=sample_id,
                roots=roots,
                top_n_each=args.top_n_each,
...
def build_arg_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="QA from two retrieval routes (topN each, dedup by content).")
    ...
    p.add_argument("--top-n-each", type=int, default=20)
    p.add_argument("--max-merged", type=int, default=40)
    p.add_argument("--base-url", required=True)
    p.add_argument("--api-key", default="EMPTY")
    p.add_argument("--model-name", default="gpt-4.1-mini")
