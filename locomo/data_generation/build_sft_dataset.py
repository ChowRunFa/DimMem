#!/usr/bin/env python3
"""
将 (conversation_window, extracted_memories) 对整理为 SFT 训练数据集。

输出格式: JSONL，每行一个样本，包含 messages 字段 (OpenAI chat format):
  [
    {"role": "system", "content": "<system_prompt>"},
    {"role": "user",   "content": "<extraction_prompt_with_conversation>"},
    {"role": "assistant", "content": "<memories_json>"}
  ]

用法:
    python build_sft_dataset.py \
        --conv-dir   results/generated_conversation \
        --mem-dir    results/generated_conversation_memories \
        --output     results/sft_memory_extraction.jsonl
"""
...
def main():
    parser = argparse.ArgumentParser(description="Build SFT dataset from conversation-memory pairs")
    parser.add_argument("--conv-dir", required=True,
                        help="Directory with generated conversations (*/windows/*.txt)")
    parser.add_argument("--mem-dir", required=True,
                        help="Directory with extracted memories (*/memories/*.json)")
    ...
    with open(args.output, "w", encoding="utf-8") as out_f:
        for mem_path in mem_files:
            # Parse path: mem_dir/ConvName/memories/window_XXXX.json
            ...
            # Build SFT sample: system=指令, user=对话原文, assistant=格式化JSON
